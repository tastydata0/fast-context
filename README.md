...
graph LR
    ...
    %% Узлы
    Req(Входящий HTTP):::input -->|X-App-Trace-Id| Mid[Middleware]:::core
    
    subgraph Service [Микросервис]
        direction TB
        Mid -->|Превращает в<br/>trace_id| Ctx((ContextVars)):::core
        Ctx -.->|Авто-проброс| Log[Loguru]:::core
        Ctx -->|Авто-проброс| Client[Httpx Client]:::core
    end
    
    Client -->|X-Out-Trace-Id| Ext(Внешний API):::out

    %% Ссылка на стиль для Service (опционально)
